...
The daily analysis workflow is defined in `.github/workflows/analyze_reddit.yml`. It analyzes completed snapshots into a unified source-linked evidence ledger, a customer-problem inventory, concise cross-stream synthesis, and an action/watchlist section; validates staged Markdown and its media-review ledger; then commits `reports/reddit/`. It uses a low-cost synthesis model first and escalates only failed validation. Configure the `COPILOT_PAT` Actions secret before enabling it.
